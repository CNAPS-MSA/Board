...
import com.skcc.board.config.KafkaProperties;
...
class BoardKafkaResourceIT {
    ...
    void setup() {
        ...
        BoardKafkaResource kafkaResource = new BoardKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/board-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/board-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
